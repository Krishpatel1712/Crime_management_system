...
<html>
   <head>
      ...
      <title>Officers Directory | Criminal Management System</title>
      ...
      <style>
         ...
         .list-card { width: 95%; margin: 20px auto; background: white; padding: 30px; border-radius: 25px; box-shadow: 0 15px 50px rgba(0,0,0,0.1); text-align: left; }
         .list-card h2 { text-align: center; color: #1a237e; margin-bottom: 30px; font-weight: 900; letter-spacing: 1px; }
         
         .officer-table-v3 { width: 100%; border-collapse: collapse; background: white; }
         .officer-table-v3 th { background: #1a237e; color: white; padding: 18px; text-align: center; border: 1px solid #ddd; font-size: 13px; font-weight: 900; text-transform: uppercase; }
         .officer-table-v3 td { padding: 15px; border: 1px solid #eee; text-align: center; font-size: 14px; color: #333; font-weight: 600; }
         .officer-table-v3 tr:nth-child(even) { background: #f9f9fb; }
         .officer-table-v3 tr:hover { background: #f1f4ff; }
         
         .badge-role-v3 { background: #ffebee; color: #d32f2f; padding: 5px 12px; border-radius: 6px; font-weight: 800; font-size: 11px; display: inline-block; }
         .badge-weapon-v3 { background: #e3f2fd; color: #1565c0; padding: 5px 12px; border-radius: 6px; font-weight: 800; font-size: 11px; display: inline-block; }
   ...
   <body>
      ...
      <div class="navbar-custom">
          <ul>
             ...
             <li><a href="offList.php" class="active">List of Officers</a></li>
      ...
      <div class="list-card">
          ...
          <table class="officer-table-v3">
             ...
             <tbody>
                <?php
                   $sql = "SELECT * FROM `officer` ORDER BY offName ASC";
                   $result = mysqli_query($db, $sql);
                   while($row = mysqli_fetch_assoc($result)){
                       echo "<tr>
                               <td><b>".$row['offName']."</b></td>
                               <td>#".$row['offID']."</td>
                               <td>".$row['contact']."</td>
                               <td>".$row['gender']."</td>
                               <td><span class='badge-weapon-v3'>".$row['weapon']."</span></td>
                               <td><span class='badge-role-v3'>".$row['role']."</span></td>
                             </tr>";
                   }
                ?>
             </tbody>
      ...
      <div class="ai-box-widget">
          <div class="ai-window" id="aiWindow">
              <div class="ai-chat-body" id="aiChatBody">
                  <div style="background:white; padding:15px; border-radius:10px; border:1px solid #e5e5e5; font-size:14px; font-family: sans-serif;">Hello Officer! I'm your Neural Assistant. Ask me anything about the personnel list.</div>
              </div>
